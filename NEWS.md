...
- `HVFInfo` and `SVFInfo` now say which method has not been run when its information is missing. `SVFInfo` also reports missing metrics with the default `status = FALSE`, instead of returning an empty table ([satijalab/seurat#7034](https://github.com/satijalab/seurat/issues/7034))
